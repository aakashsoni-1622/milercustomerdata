...
export default function ReturnDetailsPopup({ 
  order, 
  isVisible, 
  position, 
  onClose,
  ...
  const calculatePayableAmount = () => {
    ...
    const shippingAdjustment = parseFloat(formData.shippingAdjustment) || 0;
    return Math.max(0, totalAmount - shippingAdjustment);
  };
    ...
    try {
      ...
      const response = await fetch('/api/orders/create-v2', {
        ...
        body: JSON.stringify({
          ...
          shippingAdjustment: formData.shippingAdjustment,
      ...
      if (result.success) {
        ...
        alert(`Failed to update return details: ${result.error}`);
      }
    } catch (error) {
      console.error('Error updating return details:', error);
      alert(`Error updating return details: ${error instanceof Error ? error.message : 'Please try again.'}`);
    } finally {
  ...
  const returnStatusOptions = ["INPROCESS", "PAID", "COMPLETED"];
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          {isEditing ? (
            ...
            <p className="text-sm text-gray-900">{formData.returnReason || 'N/A'}</p>
          )}
        </div>
        ...
        <div>
          ...
          {isEditing ? (
            ...
            <p className="text-sm text-gray-900">{formData.returnStatus || 'N/A'}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            {isEditing ? (
              <input
                type="number"
                value={formData.shippingAdjustment || ''}
                onChange={(e) => handleInputChange('shippingAdjustment', e.target.value)}
                min="0"
                step="0.01"
                className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            ) : (
              <p className="text-sm text-gray-900">₹{formData.shippingAdjustment || '0.00'}</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-sm text-gray-900 font-medium">₹{calculatePayableAmount().toFixed(2)}</p>
          <p className="text-xs text-gray-500">
            (Total: ₹{order?.totalAmount || 0} - Shipping: ₹{formData.shippingAdjustment || 0})
          </p>
        </div>
